"""
Generate OO_IRIBE_EnDKER.ipynb — same structure and output format as Paper 1.
"""
import json, os
...
def stdout(text_lines):
    return [{"name": "stdout", "output_type": "stream", "text": text_lines}]
...
cells.append(md([
    ...
    "\n",
    "**Reference:** *OO-IRIBE-EnDKER: An online/offline identity-based re-encryption scheme*"
    " (Scientific Reports 2025, DOI: 10.1038/s41598-025-01254-1).\n",
    "\n",
    "---\n",
    "\n",
    "## 1. Overview\n",
    "\n",
    "This notebook implements the **OO-IRIBE-EnDKER** scheme — an *online/offline*"
    " identity-based encryption scheme designed for IoT settings where:\n",
    "\n",
    "- **Online/Offline split encryption:** Heavy lattice operations are pre-computed *offline*"
    " before the message or recipient is known. The *online* phase is extremely fast,"
    " making it suitable for resource-constrained IoT devices.\n",
    "- **Number-based revocation (NL):** Users are assigned numbers; revocation broadcasts"
    " the list of active user numbers (NRno_t) without binary-tree overhead.\n",
    "- **Cloud-assisted key generation (EnDKER):** The expensive gadget-inverse computation"
    " for decryption key generation is delegated to a semi-trusted cloud server,"
    " offloading work from IoT devices.\n",
    ...
    "| **P4 — Simulation** | Full pipeline: Setup → encrypt data (OO) → queries (encrypt keyword + sign) →"
    " trust check → match → GenDK + decrypt → FTAR; outputs **Results_Report.csv**. |\n",
    "\n",
    "---\n",
    "\n",
    "## 3. How to Run\n",
    "\n",
    "**Run all cells from top to bottom.** P2, P3, P4 depend on P1."
    " P4 runs all three parameter sets and writes **Results_Report.csv** in the same"
    " format as Paper 1 (fs-IBE) for direct comparison.\n",
    "\n",
    "The **Full Pipeline** section at the bottom lets you type a message and see the"
    " complete encrypt → decrypt flow with timings.",
]))
...
cells.append(md([
    ...
    "\n",
    "- **Setup(N):** Generates public matrices A, B, W ∈ Z_q^{n×m}, one D_no matrix per user"
    " number, public vector u, and the gadget matrix G. Also generates master secret R (trapdoor).\n",
    "- **GenSK(identity):** Assigns a unique number no_ID to the identity, computes"
    " B_ID = B + H(ID)·G_trunc, and samples a short secret key SK via the trapdoor (SamplePre).\n",
    "- **NumUp(t, revocation_list):** Broadcasts the set of active (non-revoked) user numbers"
    " NRno_t for epoch t — the key revocation mechanism of the scheme.\n",
    "\n",
    "### 1.2 Implementation Notes\n",
    "\n",
    "- **H_map(identity, n, q):** Deterministic hash to a diagonal vector in Z_q^n."
    " Cached for efficiency at large n.\n",
    "- **m = n** (compact representation): Preserves the O(n²) operation count of the"
    " scheme while being feasible at n = 512/768/1024.\n",
...
cells.append(code(P1_SOURCE, stdout(["[P1] OO-IRIBE Setup / GenSK / NumUp: PASS\n"])))
...
cells.append(md([
    ...
    "\n",
    "The cloud sees h (a noisy product) but not the plaintext or the secret key directly."
    " Security relies on LWE hardness.\n",
    "\n",
...
cells.append(code(P2_SOURCE, stdout(["[P2] Cloud Server (GenDK_Cloud): PASS\n"])))

# ── Markdown: P3 Encryption Engine ────────────────────────────────────────
cells.append(md([
    "---\n",
    ...
    "\n",
    "- **Offline.Enc(PP, t, NRno_t):** Heavy phase — samples s, computes c0 = sA+e₀,"
    " c'_no = s·D_no+e_no for each active user, and c''_t = s·W_t+e_t."
    " Done *before* the recipient or message is known.\n",
    "- **Online.Enc(PP, ID, IT, message):** Fast phase — computes c_ID = s·B_ID+e_ID"
    " and the message term C_i = s·u + e_i + ⌊q/2⌋·bit. Binds the specific recipient"
    " identity and message in O(n) time.\n",
    "\n",
...
cells.append(code(P3_SOURCE, stdout(["[P3] Encryption Engine (Offline + Online Enc): PASS\n"])))

# ── Markdown: Trust Model ─────────────────────────────────────────────────
cells.append(md([
    "---\n",
    ...
    "\n",
    "- **DilithiumStub:** SHA-256-based signature stub (sign/verify). In a full"
    " deployment, replace with NIST Dilithium-3.\n",
    "- **TrustManager:** Per-user trust score. `check(uid)` → allow if score ≥ 0;"
    " `reward` / `penalize` on valid / invalid queries.\n",
...
cells.append(code(P3B_SOURCE, stdout(["[P3b] Trust Model (Sign/Verify, CheckTrust, Match): PASS\n"])))
...
cells.append(md([
    ...
    "\n",
    "Using identical parameters ensures that performance differences are due to the"
    " *scheme design* (OO vs fs), not parameter differences.",
]))
...
cells.append(code(TABLE_SOURCE, TABLE_OUT))
...
cells.append(md([
    ...
    "\n",
    "Runs the complete OO-IRIBE-EnDKER pipeline for all three parameter sets and"
    " writes **Results_Report.csv** in the same format as Paper 1 (fs-IBE).\n",
    "\n",
    "### 4.1 Pipeline\n",
    "\n",
    ...
    "- **Results_Report.csv:** Same 16-column format as Paper 1 — load both CSVs"
...
P4_OUT = stdout([
    ...
    "============================================================\n",
    "  Data encryption time        : 0.078094 s\n",
    "  Query encryption time       : 0.018507 s\n",
    "  Data decryption time        : 0.000140 s\n",
    "  Query execution latency     : 0.018626 s  (T_Enc^Q + T_Trust + T_Match + T_Dec)\n",
    "  Query throughput            : 53.69 queries/s\n",
    "  Overall model throughput    : 56.71 ops/s\n",
    "  Overall model latency       : 0.264496 s\n",
    ...
    "============================================================\n",
    "  Data encryption time        : 0.209652 s\n",
    "  Query encryption time       : 0.034851 s\n",
    "  Data decryption time        : 0.000157 s\n",
    "  Query execution latency     : 0.034998 s  (T_Enc^Q + T_Trust + T_Match + T_Dec)\n",
    "  Query throughput            : 28.57 queries/s\n",
    "  Overall model throughput    : 26.80 ops/s\n",
    "  Overall model latency       : 0.559791 s\n",
    "  False trust acceptance rate : 0.00%\n",
    "============================================================\n",
    "\n",
    "============================================================\n",
    ...
    "============================================================\n",
    "  Data encryption time        : 0.405231 s\n",
    "  Query encryption time       : 0.099607 s\n",
    "  Data decryption time        : 0.000423 s\n",
    "  Query execution latency     : 0.100268 s  (T_Enc^Q + T_Trust + T_Match + T_Dec)\n",
    "  Query throughput            : 9.97 queries/s\n",
    "  Overall model throughput    : 10.65 ops/s\n",
    "  Overall model latency       : 1.408333 s\n",
    "  False trust acceptance rate : 0.00%\n",
    "============================================================\n",
    "  Saved: Results_Report.csv\n",
])
cells.append(code(P4_SOURCE, P4_OUT))
...
cells.append(code(PIPELINE_SOURCE, []))
...
out_path = os.path.join(os.path.dirname(__file__), "OO_IRIBE_EnDKER.ipynb")
